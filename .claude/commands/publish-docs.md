...
Every document in this repo is committed to `main`, deployed by the `pages` workflow, and live on GitHub Pages. A file at `<path>` in the repo must be reachable at `https://marwen-abid.github.io/<path>` with HTTP 200.
...
- Repo: `marwen-abid/docs` was renamed to `marwen-abid/marwen-abid.github.io` (user-pages repo, public). Local remote already points there.
...
2. **Watch the deploy.**
   ...
   - If no run was triggered, check `gh api repos/marwen-abid/marwen-abid.github.io/pages` (build_type must be `workflow`) and the workflow's `on:` triggers.

3. **Verify every file online.**
   For each path in `git ls-files` (skip `.github/`, `.claude/`, `.gitignore`):
   `curl -s -o /dev/null -w "%{http_code}" "https://marwen-abid.github.io/<path>"` and require `200`.
...
- [ ] Every tracked doc file returns HTTP 200 at its `https://marwen-abid.github.io/<path>` URL
